    ...
    def get(self, request, movie):
        ...
        for i in range(1, 5):
            search_url = "https://api.themoviedb.org/3/search/movie?api_key=" + \
                API_KEY + "&language=en-US&query=" + movie + "&page=" + str(i)
...
class PeopleAutocomplete(View):
    ...
    def get(self, request, people):
        ...
        API_KEY = os.environ.get("IMDB_KEY")
        for i in range(1, 5):
            search_url = "https://api.themoviedb.org/3/search/person?api_key=" + \
                API_KEY + "&language=en-US&query=" + people + "&page=" + str(i)
            response = requests.get(search_url)
            if response.status_code == 200:
                json_data = json.loads(response.text)["results"]
                results.append(json_data)
...
class MoviesSearch(View):
    ...
    def get(self, request, movie_id):
        ...
        search_url = "https://api.themoviedb.org/3/movie/" + \
            str(movie_id) + "/credits?api_key=" + API_KEY
        response = requests.get(search_url)
...
class Discover(View):
    ...
    def get(self, request):
        ...
        API_KEY = os.environ.get("IMDB_KEY")
        for i in range(1, 5):
            search_url = "https://api.themoviedb.org/3/discover/movie?api_key=" + \
                API_KEY + "&page=" + str(i)
            response = requests.get(search_url)
            if response.status_code == 200:
                json_data = json.loads(response.text)
...
class PersonSearch(View):
    ...
    def get(self, request, person_id, dpt):
        ...
        load_dotenv()
        API_KEY = os.environ.get("IMDB_KEY")
        search_url = "https://api.themoviedb.org/3/person/" + \
            str(person_id) + "/movie_credits?api_key=" + API_KEY
        response = requests.get(search_url)
        if response.status_code == 200:
            if dpt == "Acting":
                json_data = json.loads(response.text)["cast"]
            else:
                json_data = json.loads(response.text)["crew"]
            results.append(json_data)
        flat_results = [item for sublist in results for item in sublist]
        return JsonResponse(flat_results, safe=False)
    ...
    def get(self, request, *args, **kwargs):
        ...
        for composer in composers.split(','):
            response = requests.get(
                base_url + movie + " " + composer + "&limit=6")
            results = response.json()
            for result in results["data"]:
                if result["album"]["id"] not in albums_id:
                    albums_id.append(result["album"]["id"])
                    albums_list.append(
                        {'id': result["album"]["id"], "cover_url": result["album"]["cover_medium"], "type": "album"})
                else:
                    pass
        ...
        for result in results["data"]:
            if result["id"] not in albums_id:
                ...
                albums_list.append(
                    {'id': result["id"], "cover_url": result["picture_medium"], "type": "playlist"})
            else:
                pass
...
class SampleSearch(View):
    ...
    def get(self, request, query):
        ...
        response = requests.get("https://freesound.org/apiv2/search/text/?query=" + query +
                                "&fields=name,previews,duration&page_size=30&filter=duration:[1 TO 30]&token=" + SOUND_TOKEN)
        results = response.json()
        return JsonResponse(results, safe=False)
